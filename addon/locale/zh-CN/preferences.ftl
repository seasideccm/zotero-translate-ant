...
pref-untranslatedLanguage = 屏蔽的语言
pref-input = 输入
...
pref-QPS = 每秒请求次数
pref-charasLimit = 字符配额
pref-charasLimitFactor = 调节系数
pref-charasPerTime = 每次请求最大字符数
pref-hasSecretKey = 
    .label = 需要秘钥或 token
...
pref-switchService = 使用该翻译引擎或账号
pref-forbiddenService = 禁用该翻译引擎或账号
pref-recoverService =  恢复所有禁用的引擎或账号
