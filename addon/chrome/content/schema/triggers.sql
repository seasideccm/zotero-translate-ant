...
DROP TRIGGER
    IF EXISTS translation_targetTextID_targetText_targetTextID;

CREATE TRIGGER translation_targetTextID_targetText_targetTextID 
AFTER DELETE OF translateID ON translation FOR EACH ROW WHEN
(SELECT targetTextID FROM translation WHERE targetTextID 
=OLD.targetTextID) IS NULL 
BEGIN 
	DELETE FROM targetText WHERE targetTextID = OLD.targetTextID;
END; 