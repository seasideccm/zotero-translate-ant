...
export function registerShortcutsCache() {
    ...
    function cacheShortcuts(ev: KeyboardEvent, keyOptions: {
        ...
        cachedShortcuts.push(keyOptions.keyboard.getLocalized().replace(/,/g, "+").toLocaleLowerCase());
...
function customShortcut(shortcutStr: string, fnOrString: Function | string) {
    ...
    if (!shortcutFnMap) shortcutFnMap = addon.mountPoint.shortcutFnMap = new Map();
