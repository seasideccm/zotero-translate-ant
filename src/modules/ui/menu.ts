...
export function mountMenu() {
  if (document.querySelector("#" + config.addonRef + "_imgTableTool")) {
    return;
  }
  ...
  if (!parent) return;
  let widthSpace =
    toolbarMenubar!.clientWidth -
    (titlebarButtonbox!.clientWidth +
      parent.clientWidth +
      (parent as any).screenX +
      8);
  ...
  const menu = ztoolkit.UI.appendElement(menuProps, parent);
