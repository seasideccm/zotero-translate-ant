...
function getCollectionsRecursive(cOrCs?: Zotero.Collection | Zotero.Collection[]) {
    if (cOrCs == void 0) {
        const zp = Zotero.getActiveZoteroPane();
        cOrCs = zp.getSelectedCollection();
        if (!cOrCs) return [];
    }
    if (!Array.isArray(cOrCs)) {
        const ccs = cOrCs.getChildCollections();
        if (!ccs.length) return [cOrCs];
        const result = getCollectionsRecursive(ccs);
        result.push(cOrCs);
        return result;
    }

    const cccs = [];
    for (const c of cOrCs) {
        if (!c.hasChildCollections) continue;
        cccs.push(...c.getChildCollections());
    }
    if (!cccs.length) {
        return cOrCs;
    }
    const result2 = getCollectionsRecursive(cccs);
    const result3 = cOrCs.concat(result2);
    return result3;
}
...
export class Translator {
    ...
    async pdf2Note(pdfIDs: number[] = this.getPDFs()) {
    ...
    async onePdf2Note() {
        ...
        if (PdfID == void 0) return;
        await this.pdf2Note([PdfID]);
    }
    ...
    async translateOnePdf() {
        const reader = Zotero.Reader.getByTabID(Zotero_Tabs.selectedID);
        if (!reader) { return; }
        const itemID = reader._item.id;
        const contentObj = await this.contentPrepare(itemID);
        if (!contentObj) { return; }
        const docItem = await this.translateDoc(contentObj);
        await this.makeTranslation(docItem);
    ...
    getTxtCallback() {
        new ztoolkit.ProgressWindow(config.addonName)
            ...
            .show();
        const noteIDs = this.getNoteIDs();
        window.alert("成功获取笔记ID" + noteIDs);
    }
    async getHtmlMdInterconvert() {
        const betterNoteInfo = await this.getAddonInfo('Knowledge4Zotero@windingwind.com');
     ...
     * @returns 
     */
    async getNoteIDs(itemIDs?: number[] | number) {
        let items;
        if (!itemIDs) {
    ...
    async languageIdentify(sourceText: string) {
        //todo 指定排除的语言
        ...
        if (!forbiddenLang.length) {
            ...
            isTran = false;
            showInfo(getString("info-filteredByLanguageForbidden") + ": " + forbiddenLang.toString());
        }
        /*     }
          } */
        const TranslatorInfoTxt = langArr.toString();
        showInfo(TranslatorInfoTxt, 3000);
        return isTran;
    }
     ...
     * @returns 
     */
    async translateFT(type: "note" | "pdf") {

        let noteIDs: number[] = [];
        if (type == "pdf") {
            noteIDs = this.getPDFs();
        } else if (type == "note") {
            noteIDs = await this.getNoteIDs();
        }

        if (!noteIDs?.length) { return; }
        for (const noteID of noteIDs) {
            const contentObj = await this.contentPrepare(noteID);
            if (!contentObj) { continue; }
            const time1 = new Date().getTime();
            const docItem = await this.translateDoc(contentObj);
            const time2 = new Date().getTime();
            const time3 = time2 - time1;
            showInfo(getString("info-translationTIme") + (time3 / 1000).toString() + " seconds", 2000);
            await this.makeTranslation(docItem);
        }
        const serviceID = getSingleServiceUnderUse().serviceID as string;
        if (services[serviceID].hasSecretKey) {
     ...
     * @returns 
     */
    splitContent = (sourceTxt: string, emptyLine: boolean) => {
        let regex;
        if (emptyLine) {
     ...
     * @returns 
     */
    splitContentEnd = (sourceTxt: string) => {
     ...
     * @returns 
     */
    wholeSentenceSplit(sourceTxt: string, charasPerTime: number) {
        /* eslint-disable no-useless-escape */
        ...
        if (filteredArr.length == 0) {
            ...
            filteredArr = this.combineByLimit(filteredArr1, charasPerTime);
        }
        return filteredArr;
    }
     ...
     * @returns 
     */
    combineByLimit(filteredArr: string[], charasPerTime: number) {
        const resultArr = [];
        let tempStr = '';
     ...
     * @returns 
     */
    combineByLimitOneRequest(filteredArr: string[], charasPerTime: number) {
        let tempStr = '';
        const reg = /\n$/g;
     ...
     * @returns 
     */
    async contentPrepare(itemID: number) {
        ...
        //const noteHtmlArr1 = this.splitContent(noteHtml);
        ...
        if (contentEndMatch != null && contentEndMatch[0] != "") {
            ...
            if (contentEnd.includes("<table>" || "<p><img")) {
                const contentEndArrTemp = this.splitContentEnd(contentEnd);
        ...
        //正文
        const noteHtmlTrimHeadEndTailArr = this.splitContent(noteHtmlTrimHeadEndTail, true);
        let noteHtmlArr = [];
     ...
     * @returns 
     */
    async translateDoc(docItem: DocItem) {
        ...
        result = await this.translateExec(totranArr);
        ...
        /*     objArrToTranArr.filter(e => {
              if (e.type == "table" || e.type == "citation") {
                result = await this.translateExec(e.rawToTranslate);
        ...
        async function paraPostTran(docCell: DocCell) {
            ...
            tranedStr = this.modifySubSupHeading(tranedStr);
        ...
        function tableCitationPostTran(docCell: DocCell) {
            ...
            for (let i = 0; i < tranedStrArr.length; i++) {
                ...
                const temp = this.escapeString(rawToTranslate[i]);
     ...
     * @returns 
     */
    async makeTranslation(docItem: DocItem) {
        if (docItem.status == 'error') {
            const failureReason = "";
            showInfo(getString("info-translateFailure") + ": " + docItem.itemID?.toString() + ':' + failureReason);
            return;
        }
        const docCellArr = docItem.content;
        const bilingualContrast = getPref('bilingualContrast');
     ...
     * @returns 
     */
    async translateExec(sourceTxt: string | string[]) {
        let toTranArr: string[] = [];
        ...
        while (leftArr.length) {
            ...
            if (services[serviceID].isMultiParas && leftArr.length > 1
                ...
                const combineObj = this.combineByLimitOneRequest(leftArr, charasPerTime);
                ...
                const resultStr = await this.translateGo(toTran);
    ...
    translateGo = async (sourceSegment: string) => {
        let onSwitchResult;
        const serviceID = getSingleServiceUnderUse().serviceID as string;
        const secretKey = getSingleServiceUnderUse().key as string;
        let secretKeyObj;
        ...
        if (!Zotero.Streamer._socketOpen()) {
            showInfo(getString("info-networkDisconnected"), 5000);
                ...
                if (serviceAvailableCharacters < charactersTotranslate) {
                    ...
                    if (!onSwitchResult) {
                        ...
                        if (sourceSegment.length > services[serviceID].charasPerTime - 20) {
                            const sourceTxtArr = this.wholeSentenceSplit(sourceSegment, services[serviceID].charasPerTime);
                            const sourceTxtArrTrans: string[] = [];
                            for (const sourceSegment of sourceTxtArr) {
                                const result = await this.translateGo(sourceSegment) as string;
     ...
     */
    //删除行首不翻译的字符
    beginNoTransExtract(sourceTxt: string) {
        const regSS = /^[\W.]+/;
     ...
     * @returns 
     */
    endNoTransExtract(sourceTxt: string) {
        //测试后向断言,通过
        //const regSS = /(?<=[.?!"]) /g
     ...
     */
    showInfo(TranslatorInfo: string, closeTime?: number, window?: Window, closeOnClick?: boolean, closeOtherProgressWindows?: boolean) {
     ...
     * @returns 
     */
    modifySubSupHeading = (mdTxtTransResult: string) => {
