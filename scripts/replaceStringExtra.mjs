...
const replaceFromString=replaceFrom.map(reg=>reg.toString().replace(/\/[^_]*/g,""))
...
function getFileNames(localeDir,ext){
 return readdirSync(localeDir, {
    ...
    .map((dirent) => {
      ...
      const dirStringBeforBuild="`"+dir.replace("addon/chrome/content/schema/",dirPrefix)+"`"
      const str=`${name}:{        
...
export function getFileNamesRecursive(localeDir,ext){  
  const localeFolders = readdirSync(localeDir, { withFileTypes: true })
   ...
   for (const localeSubFolder of localeFolders) {
    ...
    localeSubFiles=localeSubFiles.concat( getFileNamesRecursive(localeSubDir,ext))
  }
